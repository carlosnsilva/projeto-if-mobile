...
BEGIN
	...
	LOOP
		...
		IF existe IS NULL THEN
			...
			INSERT INTO fatura(referencia, idnumero, valor_plano, tot_min_int, tot_min_ext,
			tx_min_exced, tx_roaming, total, pago) VALUES (datafatura, chip.idnumero, pl.valor,
			minIn_usado, minOut_usado, exced, roaming, total, 'S');
...
-- REQUISITO 6
CREATE OR REPLACE FUNCTION FN_DISPONIBILIZA_NUMERO(ID_CLIENTE INTEGER)
RETURNS VARCHAR
LANGUAGE PLPGSQL
AS $$
DECLARE
	...
	INFO_CLIENTE RECORD;
BEGIN
	SELECT CANCELADO INTO STATUS FROM CLIENTE WHERE idcliente = ID_CLIENTE;
	SELECT CLIENTE.IDCLIENTE,CHIP.IDNUMERO INTO INFO_CLIENTE FROM CLIENTE JOIN CLIENTE_CHIP ON CLIENTE.IDCLIENTE = CLIENTE_CHIP.IDCLIENTE JOIN CHIP ON CLIENTE_CHIP.IDNUMERO = CHIP.IDNUMERO WHERE CLIENTE.IDCLIENTE = ID_CLIENTE;
	...
	IF STATUS = 'S' THEN
		...
		RETURN 'O NUMERO ESTA DISPONIVEL PARA OUTRO CLIENTE';
	
	ELSE
		RETURN 'O CLIENTE ESTA ATIVO NA BASE DE DADOS';
	END IF;
END $$;